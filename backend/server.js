...
// --- Helpers (Asia/Colombo safe date keys) ---
function toISODateColombo(d) {
  // Force Asia/Colombo date key (YYYY-MM-DD) without needing extra libs.
...
// --- Fake DB (in-memory) ---
// Defined as templates relative to "today" (index 0, 1, 2...)
    ...
    sections: [
      {
        ...
        items: [
          { 
            ...
            tags: ["GF"],
            imageUrl: "https://lh3.googleusercontent.com/aida-public/AB6AXuCiAjJ93L07iVqenYQsw7AhB8Abvt2M10-NxwM5exXop04eb4rAHq0wB-CE4usc2EHMD80yIeJ0lWYX6dlYFUJpzoWWThrEE9FrDxwjpxtk6lPdPElQf_b1zD0J1xeG3xJpJXzIfitYBdaCZYmkb_Robr0d1M4BrlfEvXivXuRJlLsX5o76ISodH6fCRNW0FqE1P8TdTxS1Ucf1be3pMPsKRHEnGn579OGuJqTv-DoEmslYX72K2THJDoKvtzVuYk4Mt7SuUKMiQNLv"
          },
          { 
            id: "tom-2", 
            ...
            tags: ["V", "GF"],
            imageUrl: "https://lh3.googleusercontent.com/aida-public/AB6AXuDFD2hVEG3zIDd4lva6tuHrqIDDCNppSKicWP4c8d8x9AZJJLEVRPOJYtkSmvblN7OF39pylSNdxMENeAvglnxq5oNH_ya85_j3bcU4kHDw3zWhwQ_0G6aYSYmb2L2myzgXhEFHK-P8JQHVQN3sU8HrNa_iH1I6iORPPDpnVC6gYCG4fdJid6rUem9YTHScc7w9Y8H1Ehpu3QvwDWxSQGZPq0zaXDGKVA5rr-I-uSLMKKLULOxMPwUl0KGCHVOlVAZjEVLO8GrFJ305"
          }
        ]
      },
      {
        ...
        items: [
          { 
            ...
            tags: ["GF"],
            imageUrl: "https://lh3.googleusercontent.com/aida-public/AB6AXuAUb629tfUd98SobMdSAyOeW6dcS54h33syLmV1AJEmWxxuxsw2tunhAVl3YKYb5uoVzwTHr71EsQzeNOzNvBrBqHgyaWfyi1a1lQdwDNUAsVgK-Cq33VEATQleFJDxJ8D16ia2mS24m7kjv-sMFYG3Y9WGMfEIPFx_MsBI3xEkjiM4LJFT0P7gM6BvGntmQsG8aQ62xsD-Ojg_EgQ8ZYquJRnjBS4iAae6g9luz-yV24EG0dUrbgti-SNtMcdYdikFxPGWNDzo2tz3"
          },
          { 
            id: "tom-4", 
            name: "Herb Gnocchi", 
            ...
            tags: ["V"],
            imageUrl: "https://lh3.googleusercontent.com/aida-public/AB6AXuAD3Eu6Fab6PCOKSrtC7t_h7vE4OCEbkvsg7IzvH8s4BjqxTj7JoN0zVT9PNyxJ2s8oC7WxRueC7rmCZY6zpDeL-5aFP_wky2XfFhZAdJKM_NrsYI5lF76eOmn5BG5CDZACEHVQW6nwEjpXJZNhJaNd4J-aROwSMk5h_136oPYTb_5HJMB7WKHFDQ8ex9pV56UXpYSucb0nXEJH1fx-AUs8IoxsuCrCXSiB8wUvzj-vrPcZrxMGJAcCOcApSTN5-0eHwNFkBrehIplF"
          }
        ]
      }
    ]
    ...
    sections: [
      {
        ...
        items: [
          { 
            ...
            tags: ["GF"],
            imageUrl: "https://lh3.googleusercontent.com/aida-public/AB6AXuBOmRctpCmJXoEj1-Kb2IoVXd4xXnY4D-u37CbgNGiDaHGX2G51ObLfKG6bPSHx4ab1ANrRHpMl6d07bzkQVmG5tBl01Xs-pvHxnBmoBXgC3jVdjuyVEJWcWYXZRZe_Q4APLo4u8sUEjR7WkiOWKOv8ocjwG-BBq-Nr5a8no0pDYoZw-9J7S_WlgkIiSuvRNr5psZ76Bd6dz9N4MSzmzHr1suZtTaO2DJRsPq1lQvQhSu9jV4XMUEZi9vBESB-hGJ41iV33akyHUMjY"
          },
          { 
            id: "nxt-2", 
...
app.get("/api/contact", (_req, res) => res.json(CONTACT));
...
app.get("/api/menu", (req, res) => {
  ...
  for (let i = 0; i < safeDays; i++) {
    const dateKey = toISODateColombo(addDays(today, i));
    keys.push(dateKey);
    
    // Cycle through templates if we request more than 3 days
    const template = MENU_TEMPLATES[i % MENU_TEMPLATES.length];
...
app.get("/api/menu/:dateKey", (req, res) => {
  const { dateKey } = req.params;
  
  // Find relative day index from today to map to template
  const today = new Date();
  const todayStr = toISODateColombo(today);
  
  // Simple check for now: try to match generated keys for next 7 days
  let foundTemplate = null;
  for (let i = 0; i < 7; i++) {
    const k = toISODateColombo(addDays(today, i));
    if (k === dateKey) {
      foundTemplate = MENU_TEMPLATES[i % MENU_TEMPLATES.length];
      break;
    }
  }

  if (!foundTemplate) {
    return res.status(404).json({ error: "Menu not found for dateKey", dateKey });
  }
